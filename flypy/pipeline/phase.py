...
"""
flypy compiler phases (groupings of passes).
"""
...
from flypy.viz.viz import dump
...
class Phase(object):
    ...
    def apply_single(self, func, env):
        ...
        if result is not None:
            return result
        elif self.skip_opaque and env["flypy.state.opaque"]:
            return func, env

        # -- Apply phase -- #
        env['flypy.state.phase'] = phases[self.phase_name]
...
def get_cache(phase_name, env):
    cache_name = '.'.join(['flypy', phase_name, 'cache'])
...
def cache_key_argtypes(func, env):
    return (func, tuple(env["flypy.typing.argtypes"]))
...
def dpp_llvm_phase(func, env):
    ...
    dependences = [d for d in env['flypy.state.dependences']
                   if d not in cache]
...
def codegen(func, env):
    return _target_codegen_map[env['flypy.target']](func, env)
